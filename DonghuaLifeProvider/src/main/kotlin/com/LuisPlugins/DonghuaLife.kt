...
class DonghuaLifeProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select("a[href*='/donghuas/']").mapNotNull { it.animeFromElement() }
...
override suspend fun loadLinks(
    ...
    document.select("script").amap { scriptEl ->
        ...
        extractPackedBlocks(scriptText).amap { block ->
            val unpacked = unpackPACKED(block)
                .replace("diasfem", "embedsito")
            ...
            val iframeSrc = Regex("""src=\\'(https?://[^'\\]+)\\'""").find(unpacked)?.groupValues?.get(1)
            ...
            if (unpacked.contains("tamamo_player") || unpacked.contains("api_donghua")) {
                ...
                val slug = Regex("""["']slug["']\s*:\s*["']([^"']+)["']""").find(unpacked)?.groupValues?.get(1)
                    ?: Regex("""slug=["']([^"']+)["']""").find(unpacked)?.groupValues?.get(1)
                    // Fallback: parámetro base64 largo en el código
                    ?: Regex("""slug['"\s:]+([A-Za-z0-9+/=]{20,})""").find(unpacked)?.groupValues?.get(1)
                ...
                if (!slug.isNullOrEmpty() && !slug.contains("slug")) {
                    ...
                    val key = try {
                        // Formato array: [{"url":"..."}]
                        apiResp.substringAfter("\"url\":\"").substringBefore("\"")
                            .takeIf { it.isNotEmpty() && it.length > 10 }
                    } catch (_: Exception) { null }

                    if (!key.isNullOrEmpty()) {
                        val playerUrl = "https://www.mdnemonicplayer.xyz/nemonicplayer/dmplayer.php?key=$key"
                        val playerResp = app.get(
                            playerUrl,
                            headers = refererHeaders,
                            allowRedirects = false
                        ).text
                        val vidID = Regex("""video\W.*?["']([a-zA-Z0-9_\-]{4,}?)["']""")
                            .find(playerResp)?.groupValues?.get(1)
            ...
            if (unpacked.contains("asura_player")) {
                ...
                val redirectorUrl = Regex("""https?://[^\s"'<>)\\]*redirector\.php[^\s"'<>)\\]*""")
                    .find(unpacked)?.value
                    ?: Regex("""file\s*:\s*["']([^"']+\.m3u8[^"']*)["']""")
    ...
    document.select("iframe[src], iframe[data-src]").amap { iframe ->
        val src = (iframe.attr("src").takeIf { it.isNotEmpty() }
...
/**
 * Extrae todos los bloques eval(function(p,a,c,k,e,...){...}(...))
 * de un string, usando balance de paréntesis para no cortar en medio.
 */
...
/**
 * Desempaqueta el formato P,A,C,K,E,D.
 * Soporta base ≤36 (toString36) y base 62 (alfabeto extendido).
 *
...
private fun unpackPACKED(packed: String): String {
    ...
    p = Regex("""\b([0-9a-zA-Z_$]+)\b""").replace(p) { mr ->
